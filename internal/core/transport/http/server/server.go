...
func NewHTTPServer(config Config, log *core_logger.Logger, middleware ...middleware.Middleware) *HTTPServer {
	return &HTTPServer{config: config, mux: http.NewServeMux(), log: log, middleware: middleware}
}
...
func (h *HTTPServer) Run(ctx context.Context) error {
	mux := middleware.ChainMiddleware(h.mux,
		h.middleware...,
	)
	server := &http.Server{
		Addr:    h.config.Addr,
